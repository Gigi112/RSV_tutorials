...
## Section outlines
\
- In the first section, we introduces the background knowledge of transmission dynamic models of RSV.
\
\
- In the second section, we will learn how to run transmission model with all parameters known. 
\
\
- In the third section, we will learn to use maximum a posteriori / maximum likelihood estimation (MLE) to estimate the unknown parameters (one value for each unknown parameter). 
\
\
- In the fourth section, we will learn to use STAN, a probabilistic programming language for statistical inference^[https://en.wikipedia.org/wiki/Stan_(software)], to get the 95% credible intervals from the posterior samples of these unknown parameters.   



## Section 1: prerequisite readings (references)

This age structured transmission dynamic model of RSV was first developed by Ginny Pitzer in her publication: [Environmental Drivers of the Spatiotemporal Dynamics of Respiratory Syncytial Virus in the United States](https://doi.org/10.1371/journal.ppat.1004591). With the help of Dan Weinberger, we translated Ginny's Matlab codes to R codes. We further developed/modified this model, resulting in several mechanistic models corresponding to a variety of scenarios. Please refer to:

- [Estimation of the Timing and Intensity of Reemergence of Respiratory Syncytial Virus Following the COVID-19 Pandemic in the U](https://doi.org/10.1001/jamanetworkopen.2021.41779)
- [Predicted effectiveness of vaccines and extended half-life monoclonal antibodies against RSV hospitalizations in children](https://doi.org/10.1038/s41541-022-00550-5)
- [Year-round RSV Transmission in the Netherlands Following the COVID-19 Pandemic - A Prospective Nationwide Observational and Modeling Study](https://doi.org/10.1101/2022.11.10.22282132)
...
![Ginny's model](/Users/zhezheng/Library/CloudStorage/Box-Box/RSV_tutorials/RSV_tutorials/1_MSIS_model_explain/RSV_msis_model_structure.png)
...
- Each grey compartment correspond to a disease status. These statuses are **observable**. These outputs of transmission dynamic models will be used for the model calibration.
\
\
...
## Section 2: run transmission model with all parameters known
\
...
## Section 3: use maximum a posteriori (variation of MLE) to estimate the unknown parameters
...
## Section 4: Stan model to estimate posterior samples

Before we start, please learn the [Rstan basic](https://github.com/Gigi112/RSV_tutorials/tree/main/1_MSIS_model_explain/Introduction of Rstan.pptx) from the separated tutorials in the same file path. 
\
\
