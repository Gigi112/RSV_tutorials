immuno_epi_model <- function(t,y,parms, time.step='week'){
  ...
  v=parms$cover # indicator function for when the vaccine will be administered * vaccine coverage
  RR=parms$RR # relative risk of infection after vaccination; equal to 1 before vaccine introduction
  ...
  dy[,'S3'] <- gamma3*I3 +  
    ...
    c(rep(0,18),v*period.birth.rate[1]*sum(States),rep(0,2)) -
    (mu + um)*S3 + 
  ...
  dy[,'V1'] <- c(rep(0,18),v*period.birth.rate[1]*sum(States),rep(0,2)) -
